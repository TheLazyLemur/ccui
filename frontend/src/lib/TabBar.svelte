...
<div class="flex gap-1">
  <button
    on:click={() => dispatch('tabChange', 'chat')}
    class="px-3 py-1.5 text-sm transition-colors {activeTab === 'chat' ? 'bg-ink text-paper' : 'text-ink-medium hover:text-ink border border-ink-faint'}"
  >Chat</button>
  <button
